...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  chrome.storage.local.get(['captureData'], (result) => {
    ...
    if (data.extractedLinks && data.extractedLinks.length > 0) allLinks = allLinks.concat(data.extractedLinks);
    if (data.pageContext && data.pageContext.jobLinks) allLinks = allLinks.concat(data.pageContext.jobLinks);
    if (allLinks.length > 0) document.getElementById('captured-url').value = [...new Set(allLinks)].join(', ');
...
document.getElementById('btn-check').addEventListener('click', async () => {
  ...
  const stored = await new Promise((resolve) => chrome.storage.local.get(['captureData'], resolve));
  const pageContext = stored.captureData?.pageContext || null;
  const payload = {
    text: text,
    links: linksArray,
    locale: "en-IN",
    ...
    pageContext,
    employerHint: pageContext?.provider ? '' : '',
  };
...
function handleStatusUpdate(data) {
  if (data.status === 'FAILED') {
    ...
    const next = data.userMessage || data.nextActions?.[0] || data.nextAction || 'Paste the message again with sender and employer details.';
    document.getElementById('loading-status').innerText = next;
    showView('loading-view');
    document.querySelector('.input-panel').style.display = 'flex';
    return;
  }
  const ready = data.status === 'COMPLETED' && data.agentStatus === 'READY' && data.verdict && data.verdict !== 'pending';
  if (ready) {
    clearInterval(pollInterval);
    renderResult(data);
  ...
  const progress = data.agentProgress || data.stage || data.status || 'Checking…';
  document.getElementById('loading-status').innerText = progress;
}
