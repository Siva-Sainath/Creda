importScripts('config.js');
const API_BASE_URL = typeof CREDA_API_BASE !== 'undefined' ? CREDA_API_BASE : 'https://x1ed4uf5q9.execute-api.ap-south-1.amazonaws.com';

chrome.action.onClicked.addListener(async (tab) => {
  if (tab.url.startsWith('chrome://')) return;

  try {
    // 1. Capture screen immediately
    const screenshotDataUrl = await chrome.tabs.captureVisibleTab(tab.windowId, { format: 'jpeg', quality: 90 });
    
    // 2. Ask content script to extract text, url and display the floating modal
    try {
      await chrome.tabs.sendMessage(tab.id, { 
        action: "toggleModal", 
        screenshot: screenshotDataUrl 
      });
    } catch (e) {
      // If content script isn't loaded yet (e.g. tab was open before extension installed), inject it first
      console.log("Content script not found, injecting dynamically...");
      await chrome.scripting.executeScript({
        target: { tabId: tab.id },
        files: ['content.js']
      });
      // Try sending the message again after injection
      await chrome.tabs.sendMessage(tab.id, { 
        action: "toggleModal", 
        screenshot: screenshotDataUrl 
      });
    }

  } catch (err) {
    console.error("Capture failed:", err);
  }
});

chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === 'submitCase') {
    submitToBackend(message.payload)
      .then(data => sendResponse({ success: true, data }))
      .catch(error => sendResponse({ success: false, error: error.message }));
    return true; // Keep the messaging channel open for async response
  }
  
  if (message.action === 'pollCase') {
    pollBackend(message.caseId, message.token)
      .then(data => sendResponse({ success: true, data }))
      .catch(error => sendResponse({ success: false, error: error.message }));
    return true; 
  }
});

async function submitToBackend(payload) {
  let screenshotKey = null;

  let screenshotSizeBytes = 0;
  if (payload.screenshotDataUrl) {
    const uploadUrlResponse = await fetch(`${API_BASE_URL}/upload-url`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ contentType: 'image/jpeg' }),
    });
    if (!uploadUrlResponse.ok) throw new Error('Failed to get upload URL');
    const { uploadUrl, key } = await uploadUrlResponse.json();
    const blob = await (await fetch(payload.screenshotDataUrl)).blob();
    screenshotSizeBytes = blob.size;
    const putResponse = await fetch(uploadUrl, {
      method: 'PUT',
      headers: { 'Content-Type': 'image/jpeg' },
      body: blob,
    });
    if (!putResponse.ok) throw new Error('Failed to upload screenshot');
    screenshotKey = key;
  }

  const casePayload = {
    offerText: (payload.text || '').trim() || '[Screenshot submitted via Creda extension]',
    links: payload.links || [],
    locale: payload.locale || 'en-IN',
    sourceChannel: 'extension',
    employerHint: payload.employerHint || '',
    pageContext: payload.pageContext || null,
    screenshotKey,
    screenshotSizeBytes,
  };

  const response = await fetch(`${API_BASE_URL}/cases`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(casePayload)
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data.message || data.nextAction || `Creda could not start the check (${response.status}).`);
  }
  return data;
}

async function pollBackend(caseId, token) {
  const response = await fetch(`${API_BASE_URL}/cases/${caseId}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      'X-Case-Token': token
    }
  });
  
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data.message || data.nextAction || `Creda could not load this case (${response.status}).`);
  }
  return data;
}
